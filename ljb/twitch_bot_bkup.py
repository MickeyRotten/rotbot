...
Core runtime:
 • loads config
 • ensures Twitch tokens via oauth.py
 • builds Bot class with addons
 • exposes main() that bootstrap.py calls
"""

from __future__ import annotations
import os, json, asyncio, time, logging, datetime, signal
...
DEFAULT = {
    "client_id": "rm7pq6zqelnnhotyisfkg39kwxzx2q",
    "client_secret": "z3hsve174xm3ilqnzse9zj2ywqp25o",
    "streamer_access_token": None,
    "streamer_refresh_token": None,
    "twitch_channel": None,
    "bot_irc_token": "oauth:ymt9o045fndtr2te49ecekwhwdebt9",
    "bot_nick": "liljuicerbot"
}

r = requests.get("https://id.twitch.tv/oauth2/validate",
                 headers={"Authorization": cfg["bot_irc_token"]})
if r.status_code != 200:
    print("❌ Invalid bot_irc_token; please generate one with chat:read & chat:edit scopes.")
    sys.exit(1)

def load_cfg():
    if not CFG_FN.exists():
        json.dump(DEFAULT, CFG_FN.open("w"), indent=2)
        return DEFAULT.copy()
...
def save_cfg(c): json.dump(c, CFG_FN.open("w"), indent=2)
...
def main():
    ...
    class LJB(commands.Bot):
        def __init__(self):
            super().__init__(token=cfg["bot_irc_token"],
                             prefix="!",
                             initial_channels=[cfg["twitch_channel"]])
            self.bot_nick = cfg["bot_nick"]
            self.cmds: dict[str, tuple] = {}
            self.pending_subs: list = []     # ← NEW, for addon subscriptions
            self.pending_tasks: list = []    # ← NEW, for post-start announcers
            self.es = None
            self.b_id = None
        ...
        async def event_ready(self):
            ...
            # 1) Helix / IDs
            twitch = await Twitch(cfg["client_id"],
                                  cfg["client_secret"])
            await twitch.set_user_authentication(
                cfg["streamer_access_token"], all_scopes,
                cfg["streamer_refresh_token"])
            self.t_api = twitch
            self.b_id = (await first(
                twitch.get_users(logins=[cfg["twitch_channel"]]))).id
            ...
            # 4) register add-ons (they attach listen_* coroutines)
            for mod, folder in zip(mods, dirs):
                if hasattr(mod, "register"):
                    try: mod.register(self, folder)
                    except Exception as e: logging.error("[%s] register %s",
                                                         mod.__name__, e)
            ...
            # ── wait for every add-on subscription BEFORE starting WS
            if hasattr(self, "pending_subs"):
                await asyncio.gather(*self.pending_subs)
            if hasattr(self, "pending_tasks"):
                for task in self.pending_tasks:
                    asyncio.create_task(task)

            # 5) monitor reconnects
    ...
    async def _shutdown():
        ...
        try:
            if bot.es: await bot.es.stop()
            if hasattr(bot, "t_api"): await bot.t_api.close()
        finally:
            await bot.close()
